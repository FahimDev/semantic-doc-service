...
class Embedder(Protocol):
    ...
    # Sequence is used instead of list so callers can pass lists, tuples, or other ordered
    # collections.
    @property
    def name(self) -> str: ...
...
class CacheMatch:
    # A tiny immutable value object is ideal here because cache lookups should be cheap and
    # predictable.
    payload_json: str
    distance: float
...
class SemanticCache(Protocol):
    # Sequence lets the cache accept any ordered embedding container, not just one concrete list
    # type.
